...
두 값이 모두 문자열이면 b < c와 마찬가지로 사전순으로 비교가 수행됩니다(사전과 같이 사전순).
...
클로저는 함수 실행이 끝난 후에도 "기억"하고 함수의 범위(변수)에 계속 액세스하는 방법으로 생각할 수 있습니다.
...
JavaScript에서 클로저의 가장 일반적인 사용법은 모듈 패턴이다.
...
"동작위임(delegate)"이라고 하는 패턴입니다. 여기에서 연결된 개체가 필요한 동작의 일부에 대해 하나에서 다른 개체로 위임할 수 있도록 의도적으로 디자인합니다.

---

## Old & New

JS에는 새로운 추가기능이 있으며 반드시 이전 브라우저에서 사용할 수 잇는것은 아니다.

최신 JS를 구형 브라우저에 가져오는데 사용할 수 있는 두가지 주요 기술이 있다.

폴리필링, 트랜스파일링.
Polyfilling, Transfiling
...
호출은 브라우저 내부로 메시지를 보내고 메시지 상자 그리기 및 표시를 처리한다.
...
this 키워드는 해당 함수가 실행되는 방식에 따라 동적으로 바인딩 되며 이 바인딩을 이해하고 완전히 결정하기 위한 네 가지 간단한 규칙이 있다.

저자는 암시적 명시적 강제를 유용하고 과소평가된 도구로 본다.
